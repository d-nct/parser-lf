 ...
 * @file parser.c
 *
 * @brief Dada várias expressões regulares no stdin, o programa constroi e imprime árvores.
 *
...
static RegExp * parse_regexp() {
  ...
  case '*':
    ...
    break;
  case ')':
    /* Caso do parênteses exige mais detalhe */
    if (num_parenteses_abertos == 0) {
      /* De fato, erro */
      raiseSintaxError('\n', c);
      break;
    } else {
      /* Serão fechados em parser_basico e a regexp atual é vazia */
      return new_empty();
    }
  default:
    return parse_uniao();
...
static RegExp * parse_basico() {
  ...
  /* Lidando com possíveis erros de sintaxe */
    ...
    if (c == ')') {
      if (num_parenteses_abertos == 0) {
        /* Fechar parênteses sem ter aberto é erro */
        raiseSintaxError('\n', c);
      } else {
        /* Fecha o parênteses aberto */
        exige_caractere(')');
        return new_empty();
      }
    }
